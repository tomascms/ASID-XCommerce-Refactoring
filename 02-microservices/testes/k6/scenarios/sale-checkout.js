...
export function setup() {
  ...
  const rootLoginRes = http.post(`${BASE_URL}/rest/user/authenticate`, JSON.stringify({
    username: 'root',
    password: 'root',
  }), { headers: { 'Content-Type': 'application/json' } });

  if (rootLoginRes.status !== 200) {
    throw new Error('Falha na autenticação do root no setup');
  }

  const rootToken = rootLoginRes.body.replace(/"/g, '');
